...
For Sunspots-systems to work as intended, the system is dependent on external inputs. These inputs come in the form of weather forecasts, solar radiation estimates, and electricity spot-price data.
...
Results generated by the compute stage are served to the client through the frontend module. The module's job is to run a simple Apache-like web server to allow clients to access files using the request URL in a safe way and allow the creation of web interfaces that can be integrated easily. Requests are handled asynchronously through separate pthreads that read incoming client IDs from a circular client queue that can be updated by the main hosting thread.

The web server has three different methods that can be used to handle requests from clients, sorted by priority:

### Hardcoded
Hardcoded endpoints defined in [src/frontend/endpoints.c](src/frontend/endpoints.c) (under the "Static endpoints" comment) allow the developer to create custom URL checks and fully control the response generated by the module for any kind of special behavior needed like communication with other modules. If no hardcoded endpoint matches the request URL, the module moves on to comparing against [aliases](#aliases).
...
For a deeper explanation of the frontend module, see [docs/manual/frontend.md](docs/manual/frontend.md).
