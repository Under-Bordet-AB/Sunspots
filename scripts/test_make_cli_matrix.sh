...
printf "Repo: %s\n" "$ROOT_DIR"
printf "RUN_TIMEOUT_SECONDS=%s VALGRIND_RUN_SECONDS=%s INCLUDE_HEAVY=%s INCLUDE_RUN_TARGETS=%s\n" \
  "$RUN_TIMEOUT_SECONDS" "$VALGRIND_RUN_SECONDS" "$INCLUDE_HEAVY" "$INCLUDE_RUN_TARGETS"
...
if [ "${#test_modules[@]}" -gt 0 ]; then
  for test_module in "${test_modules[@]}"; do
    ...
    run_case "run-tests M=$test_module" "0" "${MAKE_BASE[@]}" run-tests "M=$test_module"
    run_case "run-tests-valgrind M=$test_module" "0" "${MAKE_BASE[@]}" run-tests-valgrind "M=$test_module"
  done
  ...
  run_case "run-tests M=<all-test-modules-combined>" "0" "${MAKE_BASE[@]}" run-tests "M=$test_modules_csv"
  run_case "run-tests-valgrind M=<all-test-modules-combined>" "0" "${MAKE_BASE[@]}" run-tests-valgrind "M=$test_modules_csv"
fi

if [ "$INCLUDE_RUN_TARGETS" = "1" ]; then
  if [ -n "$timeout_cmd" ]; then
    for module in "${all_modules[@]}"; do
      run_case "run M=$module (timeout)" "0,124" \
        "$timeout_cmd" "${RUN_TIMEOUT_SECONDS}s" "${MAKE_BASE[@]}" run "M=$module"
      run_case "run-valgrind M=$module (timeout)" "0,124" \
    ...
    run_case "run M=<all-modules-combined> (expected non-zero)" "2" \
      "${MAKE_BASE[@]}" run "M=$all_modules_csv"
    run_case "run-valgrind M=<all-modules-combined> (expected non-zero)" "2" \
      "${MAKE_BASE[@]}" run-valgrind "M=$all_modules_csv" "VALGRIND_RUN_SECONDS=$VALGRIND_RUN_SECONDS"
  else
