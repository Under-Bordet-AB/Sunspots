...
int main(int argc, char** argv) {
    ...
    if (config_get_string(rt.worker, "api_url", api_url, sizeof(api_url)) != 0) {
        ...
        if (config_get_string(rt.common, "location.latitude", latitude, sizeof(latitude)) != 0 ||
            ...
            config_get_string(rt.worker, "current_fields", current_fields, sizeof(current_fields)) != 0) {
            (void) sssdk_shutdown(&rt);
            return EXIT_FAILURE;
        }
        int n = snprintf(api_url, sizeof(api_url), "%s?latitude=%s&longitude=%s&current=%s", api_base, latitude,
                         longitude, current_fields);
    ...
    while (g_running && sssdk_should_run(&rt)) {
        ...
        if (f_rc > 0) {
            (void) openmeteo_emit_from_json(&rt, body, 0);
        } else {
            (void) sssdk_log_warn(&rt, "openmeteo fetch failed");
        }
