...
transform_status_t transform_elprisetjustnu_price(const cJSON *input, price_data_t *out){
    ...
    out->timestamp_arr_unix = (time_t *)malloc(sizeof(time_t)*no_data_points);
    out->price_arr_SEK_per_kWh = (double *)malloc(sizeof(double)*no_data_points);
    ...
    for (int i = 0; i < no_data_points; i++){
        ...
        if (!time || !price) {
            out->timestamp_arr_unix[i] = 0;
            out->price_arr_SEK_per_kWh[i] = -1;
            continue;
        } else {
            out->timestamp_arr_unix[i] = iso8601_to_unix(time->valuestring);
            out->price_arr_SEK_per_kWh[i] = price->valuedouble;
        }
    }
